"""Рис. 6 — временной ряд медиан задержки при накладных расходах зеркалирования."""
...
from common.data import NS_TO_US, OVERHEAD_ORDER, OVERHEAD_ORDER_LEGACY, load_overhead_results
...
def plot(out_dir: Path, overhead_path: Path | None = None) -> None:
    ...
    scenarios = list(zip(OVERHEAD_ORDER, ("Зеркалирование выключено", "Зеркалирование включено")))
    try:
        next(r for name, _ in scenarios for r in payload["results"] if r["scenario"] == name)
    except StopIteration:
        scenarios = list(zip(OVERHEAD_ORDER_LEGACY, ("Зеркалирование выключено", "Зеркалирование включено")))
    ...
    for name, label in scenarios:
        ...
        if not blocks:
            continue
        color = COLORS["mirror_off"] if "off" in name else COLORS["mirror_on"]
    ...
    ax.set_title(f"Накладные расходы: задержка ответа по секундам ({steady_sec} с)")
    ax.set_xlim(1, None)
    ...
    write_caption(
        out_dir,
        stem,
        f"Рисунок 6 — Диагностический временной ряд накладных расходов зеркалирования. Одно 1-секундное окно — это все "
