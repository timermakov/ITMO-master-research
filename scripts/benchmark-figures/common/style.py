...
SCENARIO_LABELS = {
    "s0-control": "Без прогрева",
    "s-ref": "Ручной прогрев",
    "s-dw": "Динамический\nпрогрев",
    "overhead-mirror-off": "Зеркалирование\nвыключено",
    "overhead-mirror-on": "Зеркалирование\nвключено",
    "h4-mirror-off": "Зеркалирование\nвыключено",
    "h4-mirror-on": "Зеркалирование\nвключено",
}

SCENARIO_DESCRIPTIONS = {
    "s0-control": "Без прогрева: первый GET /work строит in-memory hashmap.",
    "s-ref": "Ручной прогрев: перед первым боевым запросом вызывается POST /warmup.",
    "s-dw": "Динамический прогрев: инстанс прогревается зеркалированным GET-трафиком через СДПС.",
    "overhead-mirror-off": "Зеркалирование выключено: базовая задержка ответа через proxy.",
    "overhead-mirror-on": "Зеркалирование включено: задержка ответа при shadow-трафике.",
    "h4-mirror-off": "Зеркалирование выключено: базовая задержка ответа через proxy.",
    "h4-mirror-on": "Зеркалирование включено: задержка ответа при shadow-трафике.",
}
